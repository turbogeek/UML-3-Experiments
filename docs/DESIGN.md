...
| I-35 | customization | The palettes exist for CATIA Magic only; another tool needs its own customization, and there is no shared model of "which keywords belong on which diagram kind" that both the view filters and the palettes could use |
| I-36 | customization | Detail views cannot carry a UML3 palette (E17 rule 3), and a palette button for the messaging flows (`#publishes`, `#subscribes`, `#sends`, `#handles`) is missing because a flow template needs endpoints |
