...
| I-45 | tooling | **`sample-diagrams` is flaky after a full `cameo` run.** In one run of the whole suite it failed with `DisplayExposedElements` throwing `EditorWindow.restoreOptions()` NPE for `sampleClassDiagram` and `sampleClassTable`, so nothing was drawn and no SVG was written; an immediate re-run of the same step passed 3/3. Nothing on that path had changed - `--sample-svg` loads only the library and the sample, never the customization - so the cause is state left in the MagicDraw session by the step before, which creates 15 diagrams, opens the Create View dialog and probes the palette registry. Not yet reproduced deliberately, so no fix is attempted; candidates are closing each diagram window after export (the "stop the last test before starting the next" rule), resetting the DSL cache between steps, or giving the step a project of its own (2026-09-24) |
