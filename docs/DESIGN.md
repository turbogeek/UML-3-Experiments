...
| Keyword semantics (implied specialization/subsetting/inheritance) | CATIA Magic API via `verifyImpliedSpecializations.groovy` | **44/44 against the recorded baseline, including negative controls and an element written with terse ids (E21)**; probe effects 8/8 (E15) |
...
| The sample drawn as diagrams (E23) | `cameo_check.py --open --sample-svg`, suite `sample-diagrams` | **3/3 views of `samples/uml3`**, in a SysML v2 project the run creates for itself: expected display modes (TREE, UNDEFINED, UNKNOWN), 0 overlaps, all 11 elements drawn, the detail view's 20 features drawn, `ClassTable`'s 3 exclusions absent, each keyword label counted on the drawing (`«#classType def»` 5 times, `«#association def»` 0 times on the table), and SVG **and PNG** exported and verified as images |
| Design rules | `tools/check_rules.py` | examples 0 errors; 14 rules (R01–R12, R14, R15) each proven to fire |
| Patterns and requirement evaluation (E22) | `cameo_check.py --evaluations`, `evaluateRequirements.groovy`, suite `cameo-patterns` | the Observer pattern probes load and validate; 12/12 implied relationships, including a non-conforming binding that intersects types; 4/4 evaluation verdicts, including a model-level conformance requirement and two false controls |
| UML3 palettes in CATIA Magic | `verifyPalettes.groovy` + `tools/check_palettes.py` | **10/10 views** (six compact, four detail) get their UML3 palette; all 90 distinct templated buttons copy the expected element kind and keyword; each of the 29 definition-and-usage menus shows the form its diagram kind names; the UML3 Create View dialog is active; 9 negative controls fail the check on every run |
...
CATIA Magic with the Observer pattern and five predictions, all confirmed after one probe fix. Two results reach
beyond patterns. A role bound to a non-conforming type is not an error in KerML: the role's types intersect, so the
model silently becomes stricter (UML3-PAT-011 asks for a rule). And CATIA Magic's evaluation engine evaluates
satisfy relationships, including requirements that navigate the reflective KerML library, so a design-rule can be
stated as a requirement in the model and checked by the tool: `cameo_check.py --evaluations` runs such cases against
expected verdicts, and the suite `cameo-patterns` keeps E22 as a regression. The engine returns a requirement as a
structured value whose `result` feature is the verdict.

## The sample drawn, in a project the tests make themselves (E23)

A sample that only exists as text is checked by the validator and never looked at. E23 draws
[samples/uml3](../samples/uml3) in CATIA Magic and exports SVG and PNG, which is the evidence that the semantic
keywords produced the intended UML model: the sample says nothing about how to draw itself, so a box drawn as a
class is there because `ClassDiagram`'s `@classType` filter selected it. Ten of eleven predictions were confirmed.

Two results are worth keeping. First, a run no longer depends on somebody having opened a project:
`createProjectFromTemplate` on the installation's own `templates/SysML v2` gives an empty project with the same 101
KerML root namespaces a working project starts from, found through `ApplicationEnvironment.getInstallRoot()` so that
no path on one machine is written down anywhere (`cameo_check.py --open`). Second, checking the SVG is not enough:
its text is read out of a file, which would still pass on a diagram that never rasterised, so the PNG is checked as
an image — signature, `IHDR` and a minimum size.

The run also found that a `#query calc` is drawn nowhere (I-41), which is evidence in the open question of how a
UML3 query operation should map, and that one view has a diagram before anything creates one (I-40).

One piece of harness design came out of the failure that started E23. Opening a `.mdszip` — a Teamwork Cloud export,
not a local project — raised a modal error dialog on the event dispatch thread, and because the harness runs one
script at a time, that blocked every later run, including the probe that could have said what was on screen. Work
posted to the event thread is now waited for with a timeout and reports `BLOCKED`, so the harness stays answerable
...
rendered. That was half the story: compartments are a **symbol style**, not a rendering. CATIA Magic suppresses most
of them through `CompartmentVisibilityDefinitions::UsagesAndDefCompartments`, and `Symbolic View` lets a view apply
its own style sheet (`explicitlyAppliedStyleSheets`).

`UML3DetailStyleSheet` specializes the vendor's `DefaultStyleSheet` and turns the documentation, attribute, item,
action, part, port, end and parameter compartments on for definitions and usages. Four detail view definitions apply
it and carry the palette of their compact counterpart. E19 measured the result against the tool-neutral detail
views: the same content (83,011 against 83,049 bytes of SVG for the class model, 37,260 against 37,232 for the data
model, 26,922 against 26,924 for the message schemas), now with a palette. Issue I-36 is closed, and the third E17
rule became "a detail view needs a style sheet" instead of "no detail views".
...
| I-40 | tooling | In a SysML v2 project created seconds earlier, `OnlineStoreSysUMLSampleViews::sampleClassDiagram` already has a diagram before anything creates one, while the other two sample views do not (E23 P2, reproduced in two fresh projects, so it is deterministic). The cause is not established: "the view renders `asTreeDiagram`" fits these three but contradicts E13, where six TREE views of examples/05 had no diagram. It costs nothing today, because the exporter reuses an existing diagram, but it means a test cannot assume which views start without one |
| I-45 | tooling | **A long harness session stops being able to draw rendered views, and only a restart of CATIA Magic brings it back.** After some hours and dozens of created and opened diagrams, `DisplayExposedElements` throws `EditorWindow.restoreOptions()` NPE and nothing is drawn or exported. The split is exact and repeatable across runs: **every view with a rendering fails** (8 of 8 TREE/NESTED/UNKNOWN: `domainClassDiagram`, `architectureComponents`, `deploymentDiagram`, `logicalDataModel`, `physicalSchema`, `messageSchemas`, `storePackages`, `domainClassTable`) and **every view without one passes** (4 of 4 UNDEFINED: `checkoutSequence`, `domainClassDetails`, `logicalDataModelDetails`, `messageSchemaDetails`). A rendered view is shown in an editor window; an unrendered one is drawn as a plain diagram, which is why only the first kind is affected. The same suites passed twice earlier in the same session, so it degrades rather than being wrong from the start, and the fault survives a fresh project and an undo - it is application state, not model state. `exportViewDiagrams.groovy` opened diagrams for hours and closed none, so `closeDiagrams=true` now closes each one after export (`cameo_check.py` sets it for `--svg` and `--sample-svg`); it cannot repair a session already damaged, so whether it prevents the damage can only be judged from a fresh start. **This is the most likely explanation of a user reporting that "the diagrams are not working": their session had degraded the same way** (2026-09-24) |
| I-44 | UML3Views | **A table view with no columns draws nothing.** `UML3Views::ClassTable` renders `asElementTable` and names no `columnView`, so CATIA Magic opens it with the right number of rows, no column headers and every cell blank — the filter had selected the 8 classifiers of the sample and there was nowhere to put them (2026-09-24). The standard rendering carries columns in `asElementTable::columnView`, but a standard column view cannot say how a cell is drawn, so only a tool's own column views make a readable table: `UML3CatiaMagic::'UML3 Class Table'` now supplies name, documentation and qualified name, and CATIA Magic does create a `SysMLv2:UML3 Class Table` document for it. It must redefine **both** inherited renderings (`:>> asTable, UML3Views::ClassTable::asElementTable`): specializing the vendor table and our own table view brings one rendering from each, and a view may own only one (`validateViewDefinitionOnlyOneViewRendering`, which the ANTLR validator does not check — the first version loaded cleanly and was flagged only by CATIA Magic's engine, on both the definition and the usage). Two things remain open: the tool-neutral `ClassTable` still renders an empty table in any tool, and the scripted path cannot fill or photograph a real table — `DisplayExposedElements` throws `EditorWindow.restoreOptions()` NPE and painting the main frame throws `DocumentPane.getDocumentAt` NPE while a table document is active, so the table suites below are diagram-only. Note the tell: the neutral `ClassTable` reports display mode UNKNOWN because CATIA Magic falls back to drawing it as a General View diagram, which is why the SVG/PNG checks passed on it all along |
| I-42 | tooling | **A view's window opens empty in the GUI.** Opening a view's diagram by hand shows an empty canvas and the banner "This view includes the expose relationship ... go to Display > Display Exposed Elements"; a `ClassTable` opens as a table that says "Table is outdated. Please refresh the table to continue" and draws blank rows, although its status bar already reports the right count ("Total Rows: 8"). Nothing is wrong with the model: `expose` and the filters are evaluated only when the tool is asked to display or refresh, which `exportViewDiagrams.groovy` does through `DisplayExposedElements` and the test suites therefore never see. A reader who opens a view in CATIA Magic sees nothing until they invoke it themselves, which reads as "the diagrams do not work" (2026-09-24) |
| I-43 | tooling | CATIA Magic does not bring an **already open** diagram tab to the front: `open()`, `openInActiveTab(true)` and close-then-open were all tried, so `captureDiagramWindow.groovy` capturing several views in one run screenshots whichever tab is in front and writes the same picture under different names. A view of another window kind (a table) does come forward, which hides the problem. The script now hashes each image and reports `WARN|duplicate`; capture one view per run when each picture must be its own |
| I-41 | UML3Views | A `#query calc` is drawn nowhere. `Order::isPaid` appears in no compartment of the detail view, in no SVG and in no displayed-element list, while `#operation action cancel` on the same class is drawn in the actions compartment (E23 P11): CATIA Magic's detail rendering has compartments for documentation, attributes, items, ends and actions, and none for calculations. Until this is resolved, a UML3 query operation written as a calc is invisible on every diagram, which is evidence for open point 3 of [samples/uml3/README.md](../samples/uml3/README.md): a query operation may have to map to `#operation action` with a return parameter rather than to a `calc` |
