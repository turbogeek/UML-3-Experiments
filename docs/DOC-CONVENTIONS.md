...
| D04 | Every named element has an owned `doc`. Library: every definition, usage, feature, enumeration literal and metadata attribute. Examples and requirements: every definition and named usage; parameters, use case subjects and actors, enumeration literals and named control nodes (merge, join, fork, decide) may rely on the owner's doc or a comment about them | Users of a standard library read the element, not the file |
...
| D07 | Every element named after `about` in a comment exists: names resolve from the comment's namespace outward | A comment about a misspelled or removed element silently annotates nothing |
