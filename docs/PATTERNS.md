...
* The standard is **SPMS, the Structured Patterns Metamodel Standard**, version 1.4 (formal/26-04-02, January
  ...
  pattern that way: the probe passes the ANTLR validator, name resolution and the design rules with 0 findings.
...
* Work left: (1) a pattern library (keywords, metadata, a catalog); (2) checker rules for what SysML v2 does not
  ...
  force is working on (issues SPMS15-1 and SPMS15-2). Whether SysML v2 tools can evaluate structural constraints
  inside the model (E22 P5) is the open research question.
...
| structural conformance of a role player | a requirement whose subject is a metaobject, over the reflective KerML library | `SubjectTypeOwnsNotify`, satisfied by `OrderService meta KerML::Type` (E22 P5) |
...
| `FormalizedDefinition` | requirements and constraints in the model; And/Or/Not are native Boolean expressions; `DefinitionTerminal` fragments are the pattern's own structure | native, with E22 P5 open for structural constraints |
| `PropertyToRole`, `PropertyToVar` (bindings one meta level up) | features of the reflective KerML library (`ownedFeature`, `ownedSpecialization`, ...) | native reflective library; evaluation to be tested |
...
Local evidence: 0 validator errors, 0 name findings, 0 rule findings. With `notifyObservers` removed from
`OrderService`, rule R06 reports that the class playing the observable role does not implement the role type.
A role must not be named `subject`, which is a SysML v2 keyword; the probe found that `check_names` missed this
behind prefixes, and negative test n24 now covers it.
...
| G2 | SysML v2 does not reject a role bound to a non-conforming type; it intersects the types (to be confirmed, E22 P4) | checker rule | PAT-011 |
...
| G4 | Structural constraints inside the model need tool support for evaluating expressions over metaobjects | experiment E22 P5 | PAT-015 |
