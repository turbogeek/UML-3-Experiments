...
| UML3 palettes (CATIA Magic customization) | `verifyPalettes.groovy`, `tools/check_palettes.py`, suite `catia-customization` | 29 definition-and-usage menus, focused per diagram kind (definition first on class, entity-relationship and message schema palettes, usage first on component and deployment palettes); 10/10 views (six compact, four detail) get their UML3 palette; 45 templated buttons copy the expected element kind and keyword; UML3 Create View dialog active; 7 negative controls |
